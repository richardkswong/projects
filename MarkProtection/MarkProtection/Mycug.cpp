...
{
	CUGCell cell;
	//****** Get the font's index value
	int fontID = AddFont(L"Bloomberg Fixed Unicode B", 14, 1);

	//******* Set the Grid default
	GetGridDefault(&cell);
	//	cell.SetBackColor(RGB(0, 255, 255));
	cell.SetFont(GetFont(fontID));
	...
	SetNumberRows(100);
	SetNumberCols(12); // 12 columns 
	SetSH_Width(20);
	//******** Enable Hints 
	UseHints(FALSE);

	// Names 
	QuickSetText(0, -1, _T("Prot."));		SetColWidth(0, 50); 
	QuickSetText(1, -1, _T("Special"));		SetColWidth(1, 50);
	QuickSetText(2, -1, _T("TRADES"));		SetColWidth(2, 100);
	QuickSetText(3, -1, _T("Name"));		SetColWidth(3, 240);
	QuickSetText(4, -1, _T("BQty"));		SetColWidth(4, 50);
	QuickSetText(5, -1, _T("SQty"));		SetColWidth(5, 50);
	QuickSetText(6, -1, _T("Price"));		SetColWidth(6, 60);
	QuickSetText(7, -1, _T("Time"));		SetColWidth(7, 120);
	QuickSetText(8, -1, _T("Order#"));		SetColWidth(8, 50);
	QuickSetText(9, -1, _T("Status"));		SetColWidth(9, 50);
	QuickSetText(10, -1, _T("Ext.Order#")); SetColWidth(10,60);
	QuickSetText(11, -1, _T("Source"));		SetColWidth(11,20);
...
{
	// Protection flag
	if (col == 0 && row < m_NumRows) {
		if (*vcKey == 'Y' || *vcKey == 'N' || *vcKey == 'y' || *vcKey == 'n' || *vcKey == 8) {
			return TRUE;
		}
	}
	// Special flag
	if (col == 1 && row < m_NumRows) {
		if ((*vcKey >= 'A' && *vcKey <= 'Z') || (*vcKey >= 'a' && *vcKey <= 'z') || *vcKey == ' ' || *vcKey == 8) {
			return TRUE;
		}
	}
...
int MyCug::OnEditFinish(int col, long row,CWnd *edit,LPCTSTR string,BOOL cancelFlag)
{
	...
	// Protection flag
	if (col == 0 && row < m_NumRows) {
		pView = GetParentFrame()->GetActiveView();
		pView->SendMessage(WM_SetProtection, NULL, row);
		return TRUE;
	}
	// Special flag
	if (col == 1 && row < m_NumRows) {
		pView = GetParentFrame()->GetActiveView();
		pView->SendMessage(WM_SetSpecial, NULL, row);
		return TRUE;
	}
...
/*
	...
	// Protection flag
	if (oldcol == 0 && oldrow < m_NumRows) {
		pView = GetParentFrame()->GetActiveView();
		pView->SendMessage(WM_SetProtection, NULL, oldrow);
		return TRUE;
	}
	// Special flag
	if (oldcol == 1 && oldrow < m_NumRows) {
		pView = GetParentFrame()->GetActiveView();
		pView->SendMessage(WM_SetSpecial, NULL, oldrow);
		return TRUE;
	}
*/
	return FALSE;
