...
void CMarkProtectionDoc::OnConvertSaveAs()
{
	...
	if (nPos != -1)
	{
		...
		strOutputFile = strInputFile.Mid(strPath.GetLength(), strInputFile.GetLength() - strPath.GetLength());
		if (strOutputFile.Left(6) != L"Option") {    // if editing the original file, add 'Option' to the outputfilename
	...
	if (strOutputFile == L"") 
	{
		AfxMessageBox(L"File NOT saved!", MB_ICONEXCLAMATION);
		return;
	}
